...
export const projectDocsStatus = [
  ...
  { label: "Last Updated", value: "2026-06-18" },
...
export const projectDocsSections: ProjectDocSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      "公网 Demo 版采用独立 GitHub 仓库、Vercel 前端和 Render API；public callback 使用 Render API URL，不依赖本机 Cloudflare tunnel。",
    ],
  },
  {
    title: "系统架构",
  ...
  },
  {
    ...
    items: [
      ...
      "真实链上路径以 Sign Wallet Tx / Sign Endless Web Wallet Tx 为主：钱包返回真实 txHash 后系统会自动记录 receipt；Approve & Record 只作为手动录入 txHash 或失败重试入口。",
      ...
      "Solana Mainnet transfer：默认主网小额 proposal 固定为 0.000001 SOL；签名前执行余额 + fee 预检，RPC endpoint 失败时 fallback 并写入页面日志，避免 Runtime Error overlay。",
      ...
      "公网 demo 钱包入口将 Endless Web Wallet 与 Use Luffa App 分开选择；Web Wallet 空白或不可访问时不把 App 登录当作链上完成，必须重新走二维码业务授权和真实 txHash receipt。",
      "Endless Web Wallet 稳定性：connect / transaction 请求带 30 秒 timeout，SDK modal 授权完成后应能正常关闭。",
  ...
  },
  {
    ...
    items: [
      ...
      "执行 Base Sepolia acceptance：确认 Base Sepolia、绑定钱包并签名交易；拿到真实 txHash 后页面应自动生成 receipt、explorer link 和 feedback / learning 入口，如自动记录失败再使用 Retry Record。",
      ...
      "执行 Endless Web Wallet 转账 / Reward：选择 Endless Testnet 或 Endless Mainnet，右上角 Wallet / Network 点击 Use Endless Web Wallet，生成 proposal 后点击 Sign Endless Web Wallet Tx；Task Reward 默认收款人为 Alice 的固定 Endless 地址，签名前会检查 sender EDS 余额，拿到真实 txHash 后页面应自动记录 receipt，如自动记录失败再使用 Retry Record。",
      ...
      "当前 public callback caveat：若本机 TUN / DNS 把 Cloudflare tunnel edge 解析到 198.18.*，cloudflared 可能 TLS handshake EOF，导致 lael.clawworld.eu.cc 返回 1033 / 530；此时只能继续本地钱包测试，不要扫码验收。",
      ...
      "部署公网 demo：API 部署到 Render，Frontend 部署到 Vercel，Vercel NEXT_PUBLIC_LAEL_API_URL 必须指向 Render API，Render LAEL_PUBLIC_CALLBACK_BASE_URL 也必须是 Render API URL。",
      "测试公网 demo：优先打开 Vercel 前端；页面 Runtime config 应显示 loaded from Render API，Public callback 应显示 Render API URL。若 Chrome 偶发 ERR_TUNNEL_CONNECTION_FAILED，但 Vercel / Render curl 检查正常，按本机代理或 TUN 抖动处理，不按应用故障处理。",
  ...
  },
  {
    ...
    items: [
      ...
      "公网 demo 的 Retry Record 只用于带已有真实 txHash 重试 LAEL receipt 记录；如果链上交易已成功但 receipt 显示 denied，应按 stale receipt / idempotency 记录问题排查，而不是直接判断链上失败。",
      ...
      "Cloudflare public callback 返回 1033 / 530 时不要扫码；先恢复 tunnel 并跑 npm run health:luffa-app，只有 ok=true 才生成新 QR。",
      "公网 demo 不使用本机 Cloudflare tunnel；如果页面仍显示 localhost 或 lael.clawworld.eu.cc callback，说明部署 env 配错。",
  ...
  },
  {
    ...
    items: [
      ...
      "Wallet Stability Fix Report 记录 2026-06-16 Endless Web Wallet modal / timeout、Solana 余额和 fee 预检、Solana RPC fallback、Runtime Error 防护和当前 public callback 1033 / 530 caveat。",
      "Public Demo Iteration/Test Report 记录 2026-06-18 发布后 Vercel / Render / public callback、runtime-config fallback、Check / Wake API、钱包优先级、receipt 幂等、Retry Record、Solana/EVM/Endless 问题链路和代理/TUN 排查结论。",
      "Deployment / Demo Testing docs 记录公网 demo 的 Render / Vercel 环境变量、public callback、smoke 测试和失败分类。",
...
export const projectDocsIndex: ProjectDocIndexItem[] = [
  ...
  {
    type: "Wallet Stability",
    file: "LAEL_WALLET_STABILITY_FIX_REPORT_2026-06-16.zh.md",
    note: "记录 Endless Web Wallet modal / timeout、Solana 余额和 fee 预检、Solana RPC fallback、Runtime Error 防护和当前 public callback 1033 / 530 caveat。",
  },
  {
    type: "Public Demo Iteration",
    file: "LAEL_PUBLIC_DEMO_ITERATION_TEST_REPORT_2026-06-18.zh.md",
    note: "记录公网 demo 发布后的 Vercel / Render / public callback、runtime-config fallback、Check / Wake API、钱包优先级、receipt 幂等、Retry Record、Solana/EVM/Endless 问题链路和代理/TUN 排查结论。",
  },
  {
